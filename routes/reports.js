  ...
  try {
    ...
    if (format === "pdf") {
      ...
      attendance.forEach((a) => {
        doc.text(
          `${a.date.toDateString()} - ${a.user?.name || "N/A"} (${a.user?.batch || "-"}) | ${a.status} | In: ${
            a.checkIn || "N/A"
          } Out: ${a.checkOut || "N/A"}`
        );
      });
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to generate attendance report" });
  }
});
...
router.get("/payments", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    if (format === "pdf") {
      ...
      payments.forEach((p) => {
        doc.text(
          `${p.user?.name || "N/A"} | ₹${p.amount} | ${p.feeType} | ${p.status} | Due: ${
            p.dueDate?.toDateString() || "-"
          } | Paid: ${p.paidDate ? p.paidDate.toDateString() : "N/A"}`
        );
      });
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to generate payment report" });
  }
});
...
router.get("/complaints", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to generate complaint report" });
  }
});
